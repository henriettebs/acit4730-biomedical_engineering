...
# --- REMEMBER TO CHANGE! ---
RAW_FILE        = "Tests/live_prototype.csv"       # Raw per-packet log (existing)
DAILY_FILE      = "Tests/live_prototype_total.csv"   # One row per day
DAILY_TARGET_H  = 1.0                       # Hours/day required to count as a streak day
...
def handle_notification(sender, data):
    try:
        ...
        with open(RAW_FILE, "a", newline="") as f:
            writer = csv.writer(f)
            if not file_exists or os.path.getsize(RAW_FILE) == 0: # Write header if file is new
                writer.writerow(["timestamp", "steps", "temp_max", "temp_current", "temp_min", "tempRiseWorn", "accel", "accelMean", "motionWorn", "galvanicScore", "galvanic", "worn_status", "time_worn"])
    ...
    try:
        async with BleakClient(DEVICE_ADDRESS) as client:
            ...
            try:
                ...
                print(f"Could not send date (is DATE_CHAR_UUID added to Arduino?): {e}")
